...
class MoviePopularDetailViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //TODO: Register Supplementary and Collection View Cell
    ...
    private func fetchData() {
        ServieManager.shared.getMoviePopularDetail(for: movie) { [weak self] result in
        ...
        ServieManager.shared.getMovieCredit(for: movie) { [weak self] result in
            guard let self = self else { return }
            DispatchQueue.main.async {
                switch result {
                case .failure(let error):
...
extension MoviePopularDetailViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        let headerViewModel = MovieDetailCollectionViewCellViewModel(name: movie.title ?? "",
                                                                     date: "Release Date: \(String.formattedDate(string: movie.releaseDate ?? ""))",
